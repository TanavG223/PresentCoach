...
function CalibrationPanel({ calibration, csrf, profileId, onChanged }) {
  ...
  return <section className={`calibration ${calibration.ready ? "complete" : ""}`}>
    <div className="calibration-title"><div className="step-number">{calibration.ready ? <Icon name="check" /> : step}</div><div><span className="eyebrow">OPTIONAL PERSONAL COMPARISON</span><h2>{calibration.ready ? "Baseline verified" : "Calibrate for comparison"}</h2><p>{calibration.message} Descriptive feedback works without this step.</p></div></div>
...
function FeedbackPanel({ feedback, onGenerate }) {
  ...
  if (!feedback || feedback.status !== "ready") {
    ...
    return <div className="feedback-locked"><div><Icon name={legacyLock ? "spark" : "lock"} size={24} /></div><h3>{feedback?.status === "refused_short_session" ? "Recording too short" : feedback?.status === "local_ai_unavailable" ? "Feedback failed verification" : feedback?.status === "insufficient_data" ? "Insufficient measured data" : "Feedback is ready to generate"}</h3><p>{legacyLock ? "This recording was saved under the old calibration lock. Generate a neutral evidence summary now." : feedback?.message || "Record at least 30 seconds with clear face and audio data."}</p>{legacyLock && <button className="primary generate-feedback" onClick={generate} disabled={busy}>{busy ? <><span className="spinner" /> Running local AI…</> : <><Icon name="spark" /> Generate feedback</>}</button>}{error && <div className="error" role="alert">{error}</div>}</div>;
  }
  const descriptive = feedback.source === "local_llm_verified_descriptive";
  return <div className="feedback-columns">{descriptive && <p className="feedback-mode"><Icon name="scan" size={14} /> Descriptive only—calibrate for personal-reference comparisons.</p>}<div><span className="feedback-label strength"><Icon name="check" size={15} /> {descriptive ? "Measured observations" : "Strengths"}</span>{feedback.strengths.map((claim, index) => <article key={index}><strong>{metricLabel(claim.metric)}</strong><p>{claim.text}</p><small>{formatTime(claim.timestamp_seconds)} • {claim.value} {claim.unit}</small></article>)}</div><div><span className="feedback-label improve"><Icon name="spark" size={15} /> {descriptive ? "Notable moments" : "Specific improvements"}</span>{feedback.improvements.length ? feedback.improvements.map((claim, index) => <article key={index}><strong>{metricLabel(claim.metric)}</strong><p>{claim.text}</p><small>{formatTime(claim.timestamp_seconds)} • {claim.value} {claim.unit}</small></article>) : <p className="no-claims">No evidence-backed notable moment was detected.</p>}</div>{!!feedback.insufficient_data?.length && <p className="insufficient">Insufficient data: {feedback.insufficient_data.join(", ")}</p>}</div>;
}
